...
import { getRecentBundles } from '@/app/actions/peneliti';
...
import { Folder, Lock, Unlock, Send } from 'lucide-react';
...
const STATUS_STYLES: Record<string, string> = {
  ...
  'IN_MANIFEST': 'bg-indigo-50 text-indigo-700 border-indigo-100',
};

export default function PenelitiBundleListCard({ onViewAll }: { onViewAll?: () => void }) {
  ...
  useEffect(() => {
    async function fetchBundles() {
      try {
        setIsLoading(true);
        const res = await getRecentBundles(5);
        if (res.success && res.list) {
          setBundles(res.list);
        }
      } catch (error) {
        console.error('Failed to load recent bundles:', error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div id="peneliti-bundle-list-card" className="w-full flex flex-col font-sans select-none">
      ...
      <div className="flex items-center justify-between pb-[10px] border-b-2 border-gray-200/90 mb-3 font-sans">
        <div className="flex items-center gap-1.5">
          <h2 className="text-[15px] font-bold text-[#1e2022] font-display">Daftar Bundle Terbaru</h2>
          {activeCount > 0 && (
      ...
      <div className="flex flex-col flex-1 gap-1 min-h-[160px]">
        {filteredBundles.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-6 text-gray-400">
            <p className="text-xs">Tidak ada bundle aktif</p>
          </div>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="col-span-5 min-w-0 pr-2 flex items-center gap-2">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-[10px] text-gray-400 font-semibold truncate capitalize">
                      Pembuat: {item.peneliti?.name || 'Sistem'}
                    </span>
                  </div>
                </div>
                ...
                <div className="col-span-2 flex flex-col items-end gap-1">
                  <span className={`px-1.5 py-0.5 rounded text-[8px] font-extrabold border ${statusClass} leading-none`}>
                    {item.status}
                  </span>
