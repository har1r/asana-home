...
import { getDraftBundles } from '@/app/actions/peneliti';
...
const DEFAULT_CONFIG = { icon: Folder, bg: 'bg-slate-400' };

export default function PenelitiBundleDraftCard({ onViewAll }: { onViewAll?: () => void }) {
  ...
  useEffect(() => {
    async function fetchBundles() {
      try {
        setIsLoading(true);
        const res = await getDraftBundles(5); // limit to 5
        if (res.success && res.list) {
          setBundles(res.list);
          setTotalCount(res.total || 0);
        }
      } catch (error) {
        console.error('Failed to load draft bundles:', error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div id="peneliti-bundle-draft-card" className="w-full flex flex-col font-sans select-none">
      {/* Header bar */}
      <div className="flex items-center justify-between pb-[10px] border-b-2 border-gray-200/90 mb-3 font-sans">
        <div className="flex items-center gap-1.5">
          <h2 className="text-[15px] font-bold text-[#1e2022] font-display">Bundle Terbaru</h2>
          {totalCount > 0 && (
      ...
      {/* Item list — tata letak identik dengan RecentTasksCard */}
      <div className="flex flex-col gap-1">
        {filteredBundles.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-6 text-gray-400">
            <p className="text-xs">Tidak ada draf bundle</p>
          </div>
        ) : (
          filteredBundles.map((item) => {
            const config = item.jenisPermohonan ? (JENIS_CONFIG[item.jenisPermohonan] || DEFAULT_CONFIG) : DEFAULT_CONFIG;
            ...
            return (
              ...
              >
                {/* Left: ikon kotak solid + nomor bundle & subtitle */}
                <div className="flex items-center gap-2.5 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[13px] font-bold text-[#1e2022] group-hover:text-indigo-600 transition-colors truncate">
                      {item.nomorBundle}
                    </span>

                    {/* Subtitle: tanggal • jenis layanan */}
                    <div className="flex items-center gap-1.5 text-[11px] text-gray-400 font-semibold mt-0.5">
                      ...
                      <span className={`inline-flex px-1.5 py-0.5 rounded text-[9px] font-extrabold border leading-none capitalize truncate max-w-[100px] ${tagClass}`}>
