...
interface SenderApplicationsToolbarHistoryProps {
  selectedBundle: any | null;
  bundleDisplayMode: "berkas" | "pemohon";
  onDisplayModeChange: (mode: "berkas" | "pemohon") => void;
}

export const SenderApplicationsToolbarHistory: React.FC<SenderApplicationsToolbarHistoryProps> = React.memo(({
  selectedBundle,
  bundleDisplayMode,
  onDisplayModeChange,
}) => {
  const selectedBundleNo = selectedBundle?.nomorBundle || selectedBundle?.bundleNumber || "";
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-md p-3 shadow-3xs flex flex-col sm:flex-row items-center justify-between gap-3 select-none font-sans">
      ...
      {/* Right: Controls (Mode Switcher & Excel Export) */}
      <div className="flex items-center gap-2.5 shrink-0 justify-end flex-wrap font-sans">
        ...
        {selectedBundle?.id ? (
          <a
            href={`/api/export/bundle/${selectedBundle.id}`}
            download
