...
export const SenderManifestGridHistory: React.FC<SenderManifestGridHistoryProps> = React.memo(({
  loading,
  manifestsList,
  filteredManifests,
  paginatedManifests,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3.5 font-sans">
        {loading && manifestsList.length === 0 ? (
          ...
          <div className="col-span-full py-10 text-center select-none font-sans">
            <EmptyDataAnimation
              title={searchQuery ? "Hasil Pencarian Tidak Ditemukan" : "Belum Ada Riwayat Manifest"}
              description={
                searchQuery
                  ? "Tidak ada riwayat manifest yang sesuai dengan kata kunci pencarian."
                  : "Daftar riwayat manifest pengiriman kosong saat ini."
              }
            />
          </div>
        ) : (
          paginatedManifests.map((m) => (
            <SenderManifestCardHistory
              key={m.id}
              manifest={m}
