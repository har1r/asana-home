...
import { Calendar, Lock, Unlock, Send, Loader2 } from "lucide-react";
...
export interface SenderManifestCardHistoryProps {
  manifest: any;
  isSelected?: boolean;
  searchQuery: string;
  loading?: boolean;
...
export const SenderManifestCardHistory: React.FC<SenderManifestCardHistoryProps> = React.memo(({
  manifest,
  isSelected = false,
  searchQuery,
  loading = false,
  ...
  const bundlesCount = manifest.bundleCount ?? (manifest.bundles?.length || manifest.bundle?.length || 0);
  const totalPecahanCount = manifest.pemohonCount ?? manifest.applicantCount ?? 0;

  const pengirimName =
    manifest.pengirimName ||
  ...
  return (
    ...
    >
      {/* Top Row: Nomor Manifest & Status Badge */}
      ...
      <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100 font-sans mt-auto">
        <div className="flex items-center gap-1.5 min-w-0 font-sans" title={`Pengirim: ${pengirimName}`}>
          ...
          <span className="text-[11px] font-medium text-slate-600 truncate">{pengirimName}</span>
        </div>
...
SenderManifestCardHistory.displayName = "SenderManifestCardHistory";
