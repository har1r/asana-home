...
export function PenelitiBaseHeaderSkeleton() {
  return (
    <>
      {/* TIER 1: KPI STATS STRIP (Dual Zone) */}
      <div className="bg-white border border-slate-200/90 rounded-md p-1.5 shadow-3xs select-none flex flex-col xl:flex-row gap-2 items-stretch">
        {/* Zone A Skeleton: 5 Bundle Statuses */}
        <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 bg-slate-50/50 rounded-md p-1 border border-slate-100">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="p-2.5 px-3 flex items-center justify-between gap-2 rounded-md">
              <div className="flex flex-col gap-1.5 w-full">
                ...
                <SkeletonBox width="w-10" height="h-5" rounded="rounded-sm" />
              </div>
        ...
        {/* Zone B Skeleton: 3 Revision Alert Cards */}
        <div className="xl:w-[410px] grid grid-cols-1 sm:grid-cols-3 gap-1.5 shrink-0 pl-0 xl:pl-2 xl:border-l border-slate-200/80">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="p-2.5 px-3 flex items-center justify-between gap-2 rounded-md bg-slate-100/50">
              <div className="flex flex-col gap-1.5 w-full">
                <SkeletonBox width="w-20" height="h-3" rounded="rounded-sm" />
                <SkeletonBox width="w-8" height="h-5" rounded="rounded-sm" />
              </div>
              <SkeletonBox width="w-6" height="h-4" rounded="rounded-sm" />
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/90 border-b border-slate-200">
                <th className="py-3 px-4 w-12"><SkeletonText width="w-5" height="h-2.5" /></th>
                <th className="py-3 px-4 min-w-[120px]"><SkeletonText width="w-16" height="h-2.5" /></th>
                <th className="py-3 px-4 min-w-[120px]"><SkeletonText width="w-16" height="h-2.5" /></th>
                <th className="py-3 px-4 min-w-[160px]"><SkeletonText width="w-20" height="h-2.5" /></th>
                <th className="py-3 px-4 min-w-[210px]"><SkeletonText width="w-28" height="h-2.5" /></th>
                <th className="py-3 px-4 min-w-[170px]"><SkeletonText width="w-20" height="h-2.5" /></th>
                <th className="py-3 px-4 text-center min-w-[120px]"><SkeletonText width="w-16" height="h-2.5" /></th>
                <th className="py-3 px-4 text-center w-28"><SkeletonText width="w-10" height="h-2.5" /></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {Array.from({ length: 5 }).map((_, i) => (
                <tr key={i}>
                  ...
                  <td className="py-3 px-4"><SkeletonText width="w-36" height="h-3" /></td>
                  <td className="py-3 px-4"><SkeletonText width={i % 2 === 0 ? "w-28" : "w-32"} height="h-3" /></td>
                  <td className="py-3 px-4 text-center"><SkeletonBadge width="w-20" /></td>
                  <td className="py-3 px-4 text-center"><SkeletonBox width="w-20" height="h-8" rounded="rounded-lg" /></td>
                </tr>
              ))}
            </tbody>
