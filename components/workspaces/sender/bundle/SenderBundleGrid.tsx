...
import { SenderBundleCard } from "./SenderBundleCard";

export interface SenderBundleGridProps {
  loading: boolean;
  bundlesList: any[];
  filteredBundles: any[];
  paginatedBundles: any[];
...
export const SenderBundleGrid: React.FC<SenderBundleGridProps> = React.memo(({
  loading,
  bundlesList = [],
  filteredBundles = [],
  paginatedBundles = [],
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5 font-sans">
        {loading && bundlesList.length === 0 ? (
          ...
          <div className="col-span-full py-10 text-center select-none font-sans">
            <EmptyDataAnimation
              title={searchQuery ? "Hasil Pencarian Tidak Ditemukan" : "Antrean Bundle Kosong"}
              description={
                searchQuery
                  ? "Tidak ada nomor bundle yang sesuai dengan kata kunci pencarian."
                  : "Saat ini tidak ada map bundle berkas fisik yang siap dimasukkan ke dalam manifest."
              }
            />
          </div>
        ) : (
...
SenderBundleGrid.displayName = "SenderBundleGrid";
