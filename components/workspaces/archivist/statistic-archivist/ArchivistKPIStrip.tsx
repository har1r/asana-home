...
export interface ArchivistKPIStripProps {
  metrics: ArchivistStatisticMetrics;
...
const SparklineAreaChart: React.FC<{ data: number[]; color: string; id: string }> = ({ data, color, id }) => {
  ...
  return (
    <div className="relative group/chart">
      <svg width={width} height={height} className="overflow-visible">
        <defs>
          <linearGradient id={`gradient-archivist-${id}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={color} stopOpacity="0.35" />
            <stop offset="100%" stopColor={color} stopOpacity="0.0" />
          </linearGradient>
        </defs>

        <path d={areaD} fill={`url(#gradient-archivist-${id})`} />
...
export const ArchivistKPIStrip: React.FC<ArchivistKPIStripProps> = React.memo(
  ({ metrics }) => {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-2 select-none font-sans">
        ...
        <div className="bg-white rounded-md p-4 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
          <div className="flex flex-col min-w-0 mb-3">
            ...
            <p className="text-[11px] font-normal text-slate-500 truncate">Jumlah seluruh target data</p>
          </div>
        ...
        <div className="bg-white rounded-md p-4 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
          <div className="flex flex-col min-w-0 mb-3">
            ...
            <p className="text-[11px] font-normal text-slate-500 truncate">Dokumen terarsip lengkap</p>
          </div>
          ...
          <div className="flex items-end justify-between gap-2 mt-1">
            ...
            <div className="shrink-0 pb-0.5">
              <SparklineAreaChart data={metrics.uploadedTrend} color="#10b981" id="terupload" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-md p-4 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
          <div className="flex flex-col min-w-0 mb-3">
            ...
            <p className="text-[11px] font-normal text-slate-500 truncate">Menunggu arsip digital</p>
          </div>
          ...
          <div className="flex items-end justify-between gap-2 mt-1">
            ...
            <div className="shrink-0 pb-0.5">
              <SparklineAreaChart data={metrics.pendingTrend} color="#0284c7" id="belum-upload" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-md p-4 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
          <div className="flex flex-col min-w-0 mb-3">
            <h3 className="text-sm font-bold text-slate-800 leading-tight flex items-center gap-1.5">
              <span>Perlu Re-upload</span>
            </h3>
            <p className="text-[11px] font-normal text-slate-500 truncate">Dikembalikan oleh Pengirim</p>
          </div>
          ...
          <div className="flex items-end justify-between gap-2 mt-1">
            ...
            <div className="shrink-0 pb-0.5">
              <SparklineBarChart data={metrics.reuploadTrend} color="#f59e0b" />
            </div>
          </div>
        </div>
      </div>
...
ArchivistKPIStrip.displayName = "ArchivistKPIStrip";
