            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex flex-col gap-1.5">
                    ...
                    <select
                        ...
                        disabled={loading}
                        className={getInputClass(!!formErrors.applicationType, 'cursor-pointer')}
                    >
                    ...
                    {formErrors.applicationType && (
                        <span className="text-xs text-red-600 font-normal pl-1 mt-0.5 font-sans">{formErrors.applicationType}</span>
                    )}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                    <label className="text-[13px] font-normal text-slate-700 tracking-wide font-sans flex items-center justify-between">
                        ...
                        {applicationType === 'REACTIVATION' ? (
                            ...
                            <span className={`text-xs font-mono pr-1 ${(applicationNumber || '').length === 11 ? 'text-[#00a389]' : 'text-slate-400'}`}>
                                {(applicationNumber || '').length}/11 Digit
                            </span>
                        )}
                    </label>
                    <input
                        ...
                        autoComplete="off"
                        maxLength={11}
                        placeholder={applicationType === 'REACTIVATION' ? "Otomatis" : "ZXHGJ534689"}
                    ...
                    />
                    {formErrors.applicationNumber && (
                        <span className="text-xs text-red-600 font-normal pl-1 mt-0.5 font-sans">{formErrors.applicationNumber}</span>
                    )}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <input
                        type="date"
                        id="serviceNumberDate"
                        value={serviceNumberDate}
                        onChange={(e) => onServiceNumberDateChange(e.target.value)}
                        disabled={loading}
                        className={getInputClass(!!formErrors.serviceNumberDate, 'cursor-pointer')}
                    />
                    {formErrors.serviceNumberDate && (
                        <span className="text-xs text-red-600 font-normal pl-1 mt-0.5 font-sans">{formErrors.serviceNumberDate}</span>
                    )}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <input
                        ...
                        readOnly
                        disabled
                        className="w-full bg-slate-50 border border-slate-200/90 rounded-md px-3.5 py-2.5 text-[13px] font-normal text-slate-600 cursor-not-allowed font-sans select-none"
                    />
                </div>
            </div>
        </div>
