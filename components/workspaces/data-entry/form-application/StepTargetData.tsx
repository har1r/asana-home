...
import { Trash2, Phone, Copy, UploadCloud, FileText, X } from 'lucide-react';
                ...
                {(targetData || []).map((rawItem, idx) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 font-sans">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <div className={getWhatsAppContainerClass(!!formErrors[`targetData.${idx}.whatsappNumber`])}>
                                        ...
                                        <input
                                            type="text"
                                            id={`targetData.${idx}.whatsappNumber`}
                                    ...
                                    {formErrors[`targetData.${idx}.whatsappNumber`] && (
                                        <span className="text-xs text-red-600 font-normal pl-1 mt-0.5 font-sans">
                                            {formErrors[`targetData.${idx}.whatsappNumber`]}
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col gap-1.5 sm:col-span-2">
                                    <label className="text-[13px] font-normal text-slate-700 font-sans">Nama Pemohon <span className="text-red-500">*</span></label>
                                    <input
                                        type="text"
                                        id={`targetData.${idx}.ownerName`}
                                        placeholder="BUDI SANTOSO"
                                        value={item.ownerName}
                                        onChange={(e) => onTargetItemChange(idx, 'ownerName', e.target.value.toUpperCase())}
                                        style={{ textTransform: 'uppercase' }}
                                        disabled={loading}
                                        className={getInputClass(!!formErrors[`targetData.${idx}.ownerName`])}
                                    />
                                    {formErrors[`targetData.${idx}.ownerName`] && (
                                        <span className="text-xs text-red-600 pl-1 mt-0.5">{formErrors[`targetData.${idx}.ownerName`]}</span>
                                    )}
                                </div>
                            </div>

                            {/* ROW 3: 2-COLUMN SIDE-BY-SIDE LAYOUT (PEMILIK BARU VS OBJEK PAJAK) */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 font-sans">
                                {/* KOLOM KIRI: DATA PEMILIK BARU */}
                                <div className="flex flex-col gap-5">
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input
                                            type="text"
                                            id={`targetData.${idx}.ownerAddress`}
                                            placeholder="JL. MERDEKA NO. 45"
                                            value={item.ownerAddress}
                                            onChange={(e) => onTargetItemChange(idx, 'ownerAddress', e.target.value.toUpperCase())}
                                            style={{ textTransform: 'uppercase' }}
                                            disabled={loading}
                                            className={getInputClass(!!formErrors[`targetData.${idx}.ownerAddress`])}
                                        />
                                        {formErrors[`targetData.${idx}.ownerAddress`] && (
                                            <span className="text-xs text-red-600 pl-1 mt-0.5">{formErrors[`targetData.${idx}.ownerAddress`]}</span>
                                        )}
                                    </div>

                                    <div className="grid grid-cols-3 gap-2">
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[13px] font-normal text-slate-700 font-sans">Blok Pemohon <span className="text-slate-400 text-xs">(Opsional)</span></label>
                                            <input type="text" placeholder="C2" value={item.ownerBlock} onChange={(e) => onTargetItemChange(idx, 'ownerBlock', e.target.value.toUpperCase())} style={{ textTransform: 'uppercase' }} disabled={loading} className={getInputClass(!!formErrors[`targetData.${idx}.ownerBlock`])} />
                                        </div>
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[13px] font-normal text-slate-700 font-sans">RT Pemohon <span className="text-slate-400 text-xs">(Opsional)</span></label>
                                            <input type="text" placeholder="002" value={item.ownerRt} onChange={(e) => onTargetItemChange(idx, 'ownerRt', e.target.value)} disabled={loading} className={getInputClass(!!formErrors[`targetData.${idx}.ownerRt`])} />
                                        </div>
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[13px] font-normal text-slate-700 font-sans">RW Pemohon <span className="text-slate-400 text-xs">(Opsional)</span></label>
                                            <input type="text" placeholder="008" value={item.ownerRw} onChange={(e) => onTargetItemChange(idx, 'ownerRw', e.target.value)} disabled={loading} className={getInputClass(!!formErrors[`targetData.${idx}.ownerRw`])} />
                                        </div>
                                    </div>

                                    <div className="flex flex-col gap-1.5 font-sans">
                                        <label className="text-[13px] font-normal text-slate-700 font-sans">Kecamatan Pemohon<span className="text-red-500">*</span></label>
                                        <input
                                            type="text"
                                            id={`targetData.${idx}.ownerKecamatan`}
                                            placeholder="PAKUHAJI"
                                            value={item.ownerKecamatan}
                                            onChange={(e) => onTargetItemChange(idx, 'ownerKecamatan', e.target.value.toUpperCase())}
                                            style={{ textTransform: 'uppercase' }}
                                            disabled={loading}
                                            className={getInputClass(!!formErrors[`targetData.${idx}.ownerKecamatan`])}
                                        />
                                    ...
                                    <div className="flex flex-col gap-1.5 font-sans">
                                        <label className="text-[13px] font-normal text-slate-700 font-sans">Desa Pemohon<span className="text-red-500">*</span></label>
                                        <input
                                            type="text"
                                            id={`targetData.${idx}.ownerDesa`}
                                            placeholder="KOHOD"
                                            value={item.ownerDesa}
                                            onChange={(e) => onTargetItemChange(idx, 'ownerDesa', e.target.value.toUpperCase())}
                                            style={{ textTransform: 'uppercase' }}
                                            disabled={loading}
                                            className={getInputClass(!!formErrors[`targetData.${idx}.ownerDesa`])}
                                        />
                                    ...
                                    {/* LUAS TANAH (BERADA DI KOLOM KIRI DI BAWAH DESA PEMOHON SESUAI LAYOUT) */}
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        {formErrors[`targetData.${idx}.landArea`] && (
                                            <span className="text-xs text-red-600 pl-1 mt-0.5">{formErrors[`targetData.${idx}.landArea`]}</span>
                                        )}
                                    </div>
                                </div>

                                {/* KOLOM KANAN: DATA OBJEK PAJAK */}
                                <div className="flex flex-col gap-5">
                                    <div className="flex flex-col gap-1.5">
                                        <div className="flex items-center justify-between gap-2 flex-wrap">
                                        ...
                                        <input
                                            type="text"
                                            id={`targetData.${idx}.objectAddress`}
                                            placeholder="JL. MERDEKA NO. 45"
                                            value={item.objectAddress}
                                            onChange={(e) => onTargetItemChange(idx, 'objectAddress', e.target.value.toUpperCase())}
                                            style={{ textTransform: 'uppercase' }}
                                            disabled={loading}
                                            className={getInputClass(!!formErrors[`targetData.${idx}.objectAddress`])}
                                        />
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[13px] font-normal text-slate-700 font-sans">Blok Objek Pajak <span className="text-slate-400 text-xs">(Opsional)</span></label>
                                            <input type="text" placeholder="C2" value={item.objectBlock} onChange={(e) => onTargetItemChange(idx, 'objectBlock', e.target.value.toUpperCase())} style={{ textTransform: 'uppercase' }} disabled={loading} className={getInputClass()} />
                                        </div>
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[13px] font-normal text-slate-700 font-sans">RT Objek Pajak <span className="text-slate-400 text-xs">(Opsional)</span></label>
                                            <input type="text" placeholder="002" value={item.objectRt} onChange={(e) => onTargetItemChange(idx, 'objectRt', e.target.value)} disabled={loading} className={getInputClass()} />
                                        </div>
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[13px] font-normal text-slate-700 font-sans">RW Objek Pajak <span className="text-slate-400 text-xs">(Opsional)</span></label>
                                            <input type="text" placeholder="008" value={item.objectRw} onChange={(e) => onTargetItemChange(idx, 'objectRw', e.target.value)} disabled={loading} className={getInputClass()} />
                                        </div>
                                    </div>
                                    ...
                                    {/* LUAS BANGUNAN (BERADA DI KOLOM KANAN DI BAWAH DESA OBJEK SESUAI LAYOUT) */}
                                    <div className="flex flex-col gap-1.5">
                            ...
                            {/* ROW 4: BUKTI KEPEMILIKAN & UPLOAD BERKAS */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 font-sans">
                                <div className="flex flex-col gap-1.5">
                                ...
                                <div className="flex flex-col gap-1.5 font-sans">
                                    ...
                                    <label
                                        ...
                                        className={`border border-dashed rounded-md px-3 py-2 transition-all cursor-pointer flex items-center justify-between bg-white hover:bg-[#e6f6f4]/20 group min-h-[38px] ${loading ? 'opacity-50 cursor-not-allowed border-slate-200' : 'border-slate-300 hover:border-[#00a389]'
                                            }`}
                                    >
                                        <input
