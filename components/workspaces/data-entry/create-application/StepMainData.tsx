    ...
    <div className="flex flex-col gap-5 bg-transparent animate-fadeIn font-sans">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div className="flex flex-col gap-1.5">
          ...
          <select value={applicationType} onChange={(e) => onApplicationTypeChange(e.target.value)} disabled={loading} className={getInputClass(false, 'cursor-pointer')}>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input type="text" id="applicationNumber" autoComplete="off" value={applicationNumber} onChange={(e) => onApplicationNumberChange(e.target.value.toUpperCase())} style={{ textTransform: 'uppercase' }} disabled={loading} className={getInputClass(!!formErrors.applicationNumber, 'font-mono tracking-wide')} />
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[13px] font-normal text-slate-700 tracking-wide font-sans flex items-center justify-between">
            <span>Tanggal Penyelesaian (SLA)</span>
            <span className="text-[11px] text-[#008f78] bg-[#e6f6f4] px-2 py-0.5 rounded font-normal">Kalkulasi Otomatis</span>
          </label>
