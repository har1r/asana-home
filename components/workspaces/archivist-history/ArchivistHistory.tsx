...
export default function ArchivistHistory() {
  const {
    visibleBundles,
    filteredBundles,
    hasMore,
    ...
    <div className="w-full flex flex-col gap-3 animate-fadeIn font-sans select-none">
      ...
      {loading ? (
        <div className="py-16 flex flex-col items-center justify-center gap-3">
          ...
          <span className="text-xs text-slate-500 font-medium">Memuat data riwayat digitalisasi...</span>
        </div>
      ) : visibleBundles.length === 0 ? (
        <div className="py-16 flex flex-col items-center justify-center">
          <EmptyDataAnimation
            title={searchQuery ? "Tidak ada riwayat digitalisasi yang sesuai" : "Belum ada riwayat digitalisasi"}
            description={searchQuery ? "Coba ubah kata kunci pencarian." : "Bundle yang telah selesai didigitalisasi akan tampil di sini."}
          />
        </div>
      ) : viewMode === "list" ? (
        /* ==================== 3A. LIST VIEW TABLE ==================== */
      ...
      {/* 4. "VIEW MORE" LINK AT BOTTOM LEFT */}
      {hasMore && (
