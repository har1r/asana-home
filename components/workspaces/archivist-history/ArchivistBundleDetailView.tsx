...
import { ArchivistFractionsModal } from "@/components/workspaces/archivist/modal-archivist/ArchivistFractionsModal";
import { uploadArsipDigital, toggleArchiveStatus, getBundleDetails } from "@/app/actions/archivist";
...
export interface ArchivistBundleDetailViewProps {
  bundle: any;
  onBack: () => void;
}
...
const normalizeAppItem = (item: any) => {
  ...
  return {
    ...
    arsipDigitalList: item.arsipDigital || item.archives || item.digitalArchives || [],
  };
};

export const ArchivistBundleDetailView: React.FC<ArchivistBundleDetailViewProps> = React.memo(({
  bundle,
  onBack
}) => {
  const [currentBundle, setCurrentBundle] = useState<any>(bundle);
  useEffect(() => {
  ...
  const handleUploadFile = async (
    ...
    try {
      ...
      if (dataBaruId) formData.append("dataBaruId", dataBaruId);
      formData.append("uploadMode", uploadMode);

      const res = await uploadArsipDigital(formData);
      if (res.success) {
        if (currentBundle?.id) {
          const updatedRes = await getBundleDetails(currentBundle.id);
          if (updatedRes.success && "bundle" in updatedRes && updatedRes.bundle) {
            setCurrentBundle(updatedRes.bundle);
            if (fractionTargetPermohonan) {
        ...
        alert("error" in res && res.error ? (res.error as string) : "Gagal mengunggah file.");
      }
    } catch (err: any) {
  ...
  const handleToggleArchiveStatus = async (
    ...
    try {
      ...
      if (res.success) {
        ...
        alert("error" in res && res.error ? (res.error as string) : "Gagal mengubah status berkas.");
      }
    } catch (err: any) {
      alert(err.message || "Terjadi kesalahan saat mengubah status berkas.");
    } finally {
      setLoading(false);
    }
  };

  const checkPermohonanNeedsReupload = (p: any, targetId?: string | null) => {
    const archives = p.arsipDigital || p.digitalArchives || [];
    return archives.length === 0;
  };

  const rawBundleNumber = currentBundle.bundleNumber || currentBundle.nomorBundle || '';
  const bundleNum = rawBundleNumber ? formatBundleNumber(rawBundleNumber, currentBundle.createdAt) : '—';
  const rawApplications: any[] = currentBundle.applications || currentBundle.permohonan || [];
    ...
    return normalizedApplications.flatMap((item) => {
      ...
      if (isPartial && targets.length > 0) {
        return targets.map((td: any, idx: number) => {
          const targetId = td.idTargetData || td.id;
          const filteredArchives = (item.arsipDigitalList || []).filter(
            (ad: any) => ad.dataBaruId === targetId
          );
          return {
            ...item,
            ...
            targetDataBaruId: targetId,
            arsipDigitalList: filteredArchives.length > 0 ? filteredArchives : item.arsipDigitalList,
            original: item,
          };
        });
      }
  ...
  return (
    <div className="w-full flex flex-col gap-3 animate-fadeIn font-sans select-none">
      ...
      <div className="flex flex-col gap-2 select-none">
        ...
        <div>
          <button
            ...
            title="Kembali ke Riwayat Digitalisasi"
          >
          ...
          <table className="w-full text-left border-collapse font-sans">
            ...
            <tbody className="divide-y divide-slate-200/80 text-[13px] font-normal text-slate-700">
              {filteredApplications.map((item: any) => {
                const archives = item.arsipDigitalList || [];
                return (
                  <tr
          ...
          {filteredApplications.map((item: any) => {
            ...
            return (
              <div
                key={item.uniqueRowKey || item.id}
                onClick={() => setSelectedDetailsItem(item.original || item)}
                className="group bg-white border border-slate-200/90 hover:border-slate-300 rounded-md p-4 shadow-3xs hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer flex flex-col justify-between gap-3.5 relative overflow-hidden min-h-[140px]"
              >
        ...
        >
          <div className="py-0.5">
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setFractionTargetPermohonan(activeMenuItem.original || activeMenuItem);
            ...
            >
              <FileSpreadsheet className="w-3.5 h-3.5 text-[#00a389] group-hover:text-[#008f78] transition-colors" />
      ...
      {/* 7. MODAL MUTASI PECAHAN */}
      {showFractionsModal && fractionTargetPermohonan && (
        <ArchivistFractionsModal
          isOpen={showFractionsModal}
          permohonan={fractionTargetPermohonan}
          loading={loading}
          ...
          onToggleArchiveStatus={handleToggleArchiveStatus}
...
ArchivistBundleDetailView.displayName = "ArchivistBundleDetailView";
