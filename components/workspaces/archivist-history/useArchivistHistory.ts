...
import { getArchivistHistoryBundles } from "@/app/actions/archivist";
...
export function useArchivistHistory() {
  ...
  const fetchHistory = useCallback(async (isSilent = false) => {
    ...
    try {
      const res = await getArchivistHistoryBundles();
      if (res.success && Array.isArray(res.list)) {
        setRawBundles(res.list);
      } else {
        setError(res.error || "Gagal memuat riwayat digitalisasi arsip.");
      }
    } catch (err: any) {
