...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const permohonan: any = await prisma.application.findUnique({
      where: { id }
    });
    ...
    if (permohonan.applicationType !== 'PARTIAL_MUTATION') {
    ...
    const totalPecahanLT = permohonan.targetData?.reduce((sum: number, db: any) => sum + (db.landArea || 0), 0) || 0;
    const totalPecahanLB = permohonan.targetData?.reduce((sum: number, db: any) => sum + (db.buildingArea || 0), 0) || 0;

    const sisaLT = Math.max(0, (permohonan.previousData?.[0]?.landArea || 0) - totalPecahanLT);
    const sisaLB = Math.max(0, (permohonan.previousData?.[0]?.buildingArea || 0) - totalPecahanLB);
